...
public class ProdutoDAO {
	...
	public List<Produto> listaPorData(Calendar data) {
		return manager.createQuery("select distinct(p) from Produto p join fetch p.precos precos where p.dataLancamento >= :dataLancamento", Produto.class)
				.setParameter("dataLancamento", data)
				.getResultList();
	}
	
	
	public Long somaProdutosPorData(Calendar data){
	    TypedQuery<Long> query = manager.createQuery("select count(p) from Produto p "
	    		+ "where p.dataLancamento >= :dataLancamento", Long.class);
	    query.setParameter("dataLancamento", data);
	    return query.getSingleResult();
	}	
	
	public Long somaProdutos(){
